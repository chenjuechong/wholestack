...
router.get('/list', function(req,res,next){
	// res.send('hello,goods list');  // 测试路由 链接成功页面出现'hello,goods list

	// express获取请求参数
	let page = parseInt(req.param("page")); // get 请求数据拿到数据；res.param()
...
// 启动express
// node server/bin/www 或 pm2方式 或 webstorm等
// localhost：3000/goods/   // '/goods'是app.js的一级路由，'/'是本页的二级路由
				...
				if(goodsItem){ // 若购物车商品已存在
					...
					Goods.findOne({productId:productId},function(err1,doc){
						if(err1){
							...
							if(doc){
								doc.productNum = 1; // 在Goods模型添加属性，要去models/goods.js的Schema添加这两个属性。
								doc.checked = 1;
