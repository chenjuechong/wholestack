...
<script>
	...
	import NavBread from '@/compon ents/NavBread.vue' // 面包屑
	import Modal from '@/components/Modal.vue' // 模态框
	import axios from 'axios'
	import {
		currency
	} from '@/util/currency.js' // 对价格格式化的通用方法
	...
	export default {
		...
		filters: { //定义局部过滤器
			currency: currency ,//currency.js 传过来的本就是函数
		},
		computed: { //实时计算的是属性，只不过是函数写法，data里面就不用在声明了
			checkAllFlag() {  // 是否全选属性
		...
		methods: {
			...
			delCartConfirm(item) { // 点击删除图标
				this.delItem = item;
				this.modalConfirm = true; // 关闭模态框显示
			},
			closeModal() { // 关闭模态框
			...
			editCart() { // 编辑商品
				axios.post('/users/cartEdit',{
					...
					this.$store.commit("upadateCartCount",num);
				})
			},
			toggleCheckAll() {
				var flag =!this.checkAllFlag; //声明变量取代
				this.cartList.forEach((item)=> {
					item.checked = flag?'1':'0'; //如果flag是true，checked==1，反之等于0
				})
