...
export default {
	data() {
        ...
        },
        {
          ...
          endPrice:'5000.00'
			...
			loading: false, // 往下滚动“加载图标”出现效果

			mdShow:false,  //未登入的模态框是否显示
			mdShowCart: false // 已登入模态框是否显示
		}
	},
	components: {
		NavHeader,
	...
	methods: {
			...
			axios.get("/goods/list",{
				...
				if(res.status == '0') {
					if(flag){  // true.商品数据累加
						...
						if(res.result.count == 0){  // 0条数据，不加载滚动加载方式
							this.busy = true;  //禁用
						}else{
							this.busy =false; //启用
						}
		...
		sortGoods(){ //点击排序商品
			...
			this.page =1;  // 点击价格排序后第一页开始
			this.getGoodsList(); // 重新加载数据
		},
		setPriceFiler(index){ //点击价格
		this.priceChecked = index;
		this.closePop();
		this.getGoodsList();
		},
		showFilerPop() { // 点击filterBy出现价格菜单和遮罩
			this.filerBy = true;
		...
		setTimeout(() => { // 一个滚动完成之后再加载下一个
		this.page ++;
		this.getGoodsList(true);  // 滚动加载就是累加数据，并不是只显示一页数据,需要传参去请求数据的地方判断
		}, 500);// 500ms执行()=>方法
		},
		...
		axios.post("/goods/addCart",{  // 接口设置在server/routes/goods.js
			productId:productId
				...
				{
					// alert("加入成功")
					this.mdShowCart = true;  // 加个购物车成功，成功的模态框显示

					//购物车数量加一
					this.$store.commit('updateCartCount',1);
				}else{
					// alert("msg:"+res.msg)
					this.mdShow = true;  //未登入模态框显示
				}
		})
		},
		closeModal(){  //关闭模态框
			this.mdShow = false;  //未登入模态框消失
			this.mdShowCart = false;  //未登入模态框小事
		}
	}
}
</script>
